...
public interface ProductRepository extends JpaRepository<Product,Long> {
    /*ADMIN*/
    @Query("select pageable from Product pageable")
    ...
    @Query(value = "select * from products p inner join categories c on c.category_id = p.category_id where p.category_id = ?1"
    ,nativeQuery = true)
